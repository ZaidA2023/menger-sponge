...
export class Camera {
  ...
  public pitch(radians: number, down?: boolean): void {
    ...
    //Personal choice to mimic actual FPS
    this._right = Vec3.cross(new Vec3([0, 1, 0]), this._forward).normalize(); 
  ...
  public yaw(radians: number, right?: boolean): void {
    ...
    this.rotate(new Vec3([0,1,0]), radians); 
    //Personal choice, this is much nicer imo
    this._right = Vec3.cross(new Vec3([0, 1, 0]), this._forward).normalize();
    this._up = new Vec3([0, 1, 0]);
